...
est.beta.se <- rbind(sqrt(diag(vcov(glm_naiv1))),
                   sqrt(diag(glm_simex1$variance.jackknife)), glm_MCEM1$beta.se2)
...
```{r, echo=TRUE}
gam_MCEM1 <- refitME(gam_naiv1, sigma.sq.u, W, B, se.comp = FALSE)
```
...
for(i in 1:4) {
  if (i == 1) {
    plot(gam_MCEM1$mod, select = i, ylim = c(-0.35, 0.1), xlim = range(plot_gam_naiv[[1]]$x), 
  ...
  if (i == 2) {
    plot(gam_MCEM1$mod, select = i, ylim = c(-0.25, 0.3), rug = FALSE, col = "blue", all.terms = TRUE,
         xlab = xlab.names[i], ylab = "s(Mortality counts)", lwd = 2, cex.lab = 1.3, cex.axis = 1.3,
         cex.main = 2, font.lab = 1.1, cex = 1.4, shade = T)
  ...
  if (i == 3) {
    plot(gam_MCEM1$mod, select = i, ylim = c(-0.2, 0.4), rug = FALSE, col = "blue", all.terms = TRUE,
         xlab = xlab.names[i], ylab = "s(Mortality counts)", lwd = 2, cex.lab = 1.3, cex.axis = 1.3,
         cex.main = 2, font.lab = 1.1, cex = 1.4, shade = T)
  ...
  if (i == 4) {
    plot(gam_MCEM1$mod, select = i, ylim = c(-0.06, 0.08), rug = FALSE, col = "blue", all.terms = TRUE,
